...
def format_manuscript(d):
    formatted_dict = dict.fromkeys(["title", "authors", "year", "venue", "published_url", "github_url","preprint_url", "buttons"])
    formatted_dict["title"] = d["title"]
    formatted_dict["year"] = d["year"]
    formatted_dict["venue"] = d["venue"]

    authors = [aut if aut != "me" else "<strong>Robert Schauner</strong>" for aut in d.get("authors", ["me"])]
...
def create_buttons(d):
    ...
    if d["published_url"] is not None:
        buttons.append(button(d["published_url"], "Published", "ai-archive"))

    d["buttons"] = buttons
    return d

def format_to_html(d):
    citation = f'{d["authors"]}. ({d["year"]}) <font color="#C33C54">"{d["title"]}."</font>'
    d["html"] = "<li class='list-group-item'>" + citation + "<br>" + " ".join(d["buttons"]) + "</li>"
    return d
